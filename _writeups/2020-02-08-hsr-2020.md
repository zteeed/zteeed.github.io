...
Attention: Utiliser ces outils en analysant les images indépendemment ne permet pas une reconnaissance avec précision et l'outil ne sera pas capable de faire la différence entre un `i` et un `j` par exemple. L'astuce est donc de concaténer toutes les images et d'en analyser le résultat. Pour concaténer:
...
<ul>
<li>`-crop 140x140+150+90` : redimension de l'image 140x140 pixels à partir de 140 pixel vers la droite et 80 pixels vers le bas</li>
<li>`+repage` : on supprimer les métadonnées</li>
<li>`-colorspace gray` : convertion de la couleur au gris</li>
